...
[[index]]
name = "data"
layers = ["data-local", "pypi"]

[[index]]
name = "web"
layers = ["web-local", "pypi"]
```
...
The mirror layer fetched httpx from pypi.org, cached it, and will serve the `web` route from the same copy: install
...
- Nest overlays and route several mirrors: [compose overlays](@/guides/compose-overlays.md)
- Add an upstream that needs credentials: [proxy a private mirror](@/guides/private-mirror.md)
