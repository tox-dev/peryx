...
A node records download usage in daily buckets outside request handling. In `dc` and `ha` modes, producers send sealed
...
- `repository` and `project`: repository and driver subject
- `version`: driver version identity, or an empty value when none applies
- `source`: routed upstream for a cache miss, or an empty value for local content
- `downloads` and `bytes`: request and byte totals
