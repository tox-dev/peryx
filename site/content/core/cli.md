...
accept it, without opening the data directory, binding a socket, or reaching an upstream. It runs the cross-field rules
...
or a cached index's upstream. `--ecosystem` filters `list` by an installed implementation identifier.
...
recent runs newest-first as JSON; `show` prints one run by its `jr_...` id. Ecosystem implementations may add one-shot
jobs; `reindex` rebuilds the search index; `drain` finalizes an authority's retained writes at its new home after a
failover. Every run records a durable history entry you can read back with `list` and `show`.
...
`reindex` recovers an index that incremental refresh cannot bring current, such as after a partial restore, say, or a
bug that left the index stale. It re-derives every document and republishes the index in one node-wide run recorded as a
`search_rebuild` job.
...
- [PyPI](@/ecosystems/pypi/reference/mirroring.md)
- [OCI](@/ecosystems/oci/reference/mirroring.md)
...
peryx cache list --index <index> --project <resource>
...
peryx cache size
peryx cache fsck
peryx cache purge project --index <index> --project <resource>
peryx cache purge project --index <index> --project <resource> --yes
...
`cache purge project` removes the selected resource's cached metadata and unshared implementation records. It does not
...
peryx policy dry-run --index <index> --project <resource>
```

`policy dry-run` scans cached and hosted records through the selected implementation, then prints tab-separated denial
rows:

```text
action  index  project  filename  version  rule  field  reason
serve   cache   blocked-name             project-block-list  project  project "blocked-name" is blocked
```
...
manager owns the file ([installation](@/core/installation.md)).
