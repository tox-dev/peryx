...
Ecosystem references define admission requests and client responses:

- [Python package uploads](@/ecosystems/pypi/reference/uploads.md#durable-ingress-admission)
...
One local transaction writes the published metadata, replication outbox entry, `published` outcome, and transition to
...
| `published`   | Return the stored success without another publication or outbox entry. |
