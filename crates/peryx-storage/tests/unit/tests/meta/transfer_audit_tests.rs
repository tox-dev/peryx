...
fn test_repeated_transfers_read_in_commit_order() {
    let (_dir, store) = store();
    // Recorded out of commit order; the zero-padded key sorts them by commit index on read.
...
fn test_re_recording_the_same_commit_books_one_line() {
    ...
    // A retry after a leader loss re-commits the same move; the same key overwrites rather than doubling.
