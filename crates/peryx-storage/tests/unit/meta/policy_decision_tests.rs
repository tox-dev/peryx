...
fn test_history_limit_removes_old_subjects() {
    ...
    for evaluated_at_unix in 0..20 {
        store
            .record_policy_decision_with_history_limit(
                decision(&format!("resource-{evaluated_at_unix}"), evaluated_at_unix),
                16,
            )
            .unwrap();
    }
    ...
    assert!(
        store
            .current_policy_decision(decision("resource-0", 0))
            .unwrap()
            .is_none()
    );
    assert!(
        store
            .current_policy_decision(decision("resource-4", 0))
            .unwrap()
            .is_some()
    );
}
