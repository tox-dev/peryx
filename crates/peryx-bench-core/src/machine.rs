//! Hardware medians contextualize server rates. The in-process loopback row measures socket cost, not
//! a multicore server ceiling.
...
fn sysctl(name: &str) -> Option<String> {
    sysctl_with(name, &|name| {
        std::process::Command::new("sysctl").args(["-n", name]).output()
    })
}
...
type FileMeasure = dyn Fn(&Path, usize, usize, usize) -> anyhow::Result<f64> + Sync;

async fn baselines(scratch: &Path, volumes: &[Volume], settings: ProfileSettings) -> anyhow::Result<Vec<Baseline>> {
    baselines_with(scratch, volumes, settings, &disk_write, &page_cache_read).await
}

async fn baselines_with(
    scratch: &Path,
    volumes: &[Volume],
    settings: ProfileSettings,
    disk_write: &FileMeasure,
    page_cache_read: &FileMeasure,
    ...
    Ok(vec![
        Baseline {
            ...
            single: repeat(settings.rounds, &mut || memory_copy(1, settings.memory_bytes)),
            parallel: repeat(settings.rounds, &mut || {
                memory_copy(settings.clients, settings.memory_bytes)
            }),
        },
        Baseline {
            name: "disk write".to_owned(),
            measures: format!("a sequential write to {disk}, flushed to the device"),
            single: try_repeat(settings.rounds, &mut || {
                disk_write(scratch, 1, settings.memory_bytes, settings.chunk_bytes)
            })?,
            parallel: try_repeat(settings.rounds, &mut || {
...
fn repeat(rounds: usize, measure: &mut dyn FnMut() -> f64) -> String {
    measure();
    summarize((0..rounds).map(|_| measure()).collect())
}

fn try_repeat(rounds: usize, measure: &mut dyn FnMut() -> anyhow::Result<f64>) -> anyhow::Result<String> {
    measure()?;
    (0..rounds)
        .map(|_| measure())
...
fn page_cache_read(scratch: &Path, workers: usize, bytes: usize, chunk_bytes: usize) -> anyhow::Result<f64> {
    ...
    let start = Instant::now();
    std::thread::scope(|scope| {
        // Start every reader before joining so reads overlap.
        let mut handles = Vec::with_capacity(workers);
        for _ in 0..workers {
