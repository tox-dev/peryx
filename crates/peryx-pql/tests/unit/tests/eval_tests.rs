...
fn row() -> Row {
    Row::new()
        .with("n", Value::Int(5))
        .with("s", Value::Str("numpy".to_owned()))
...
fn test_evaluate_incomparable_never_matches_ordering() {
    let row = row();
    // A type mismatch is incomparable: ordering ops are false, and equality is false too.
...
fn test_evaluate_membership() {
    ...
    let hit = Predicate::In {
        field: "s".to_owned(),
        values: vec![Literal::Str("scipy".to_owned()), Literal::Str("numpy".to_owned())],
    };
    let miss = Predicate::In {
        field: "s".to_owned(),
        values: vec![Literal::Str("scipy".to_owned())],
    };
...
fn test_evaluate_starts_with() {
    ...
    let hit = Predicate::StartsWith {
        field: "s".to_owned(),
        prefix: Literal::Str("num".to_owned()),
    };
