...
async fn test_body_completion_ignores_clean_eof_before_the_declared_length() {
    let (completed, record) = completion_probe();
    let body = on_body_complete(Body::from("hello"), 10, record);
...
async fn test_body_completion_reports_bytes_before_a_terminal_poll() {
    let (completed, record) = completion_probe();
    let body = on_body_complete(Body::from("hello"), 5, record);
...
async fn test_body_completion_reports_an_empty_declared_body_at_eof() {
    let (completed, record) = completion_probe();
    let body = on_body_complete(Body::empty(), 0, record);
...
async fn test_body_completion_ignores_failed_streams() {
    ...
    let body = on_body_complete(body, 5, record);
...
async fn test_body_completion_ignores_a_dropped_partial_consumer() {
    ...
    let mut stream = on_body_complete(body, 5, record).into_data_stream();
...
#[cfg(unix)]
#[tokio::test]
async fn test_pipelined_file_seek_error_poisons_stream() {
    use std::os::fd::OwnedFd;
    use std::os::unix::net::UnixStream;

    let (stream, _peer) = UnixStream::pair().unwrap();
    let file = std::fs::File::from(OwnedFd::from(stream));

    assert!(to_bytes(pipelined_file(file, 1, 1), usize::MAX).await.is_err());
}
...
fn completion_probe() -> (Arc<AtomicU64>, impl FnOnce(u64) + Send + 'static) {
    let completed = Arc::new(AtomicU64::new(u64::MAX));
    let recorded = Arc::clone(&completed);
    (completed, move |bytes| recorded.store(bytes, Ordering::Relaxed))
}
