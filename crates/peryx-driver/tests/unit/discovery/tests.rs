...
fn test_discovery_documents_keep_neutral_index_data() {
    ...
    assert_eq!(entry["route"], "team/packages");
...
fn test_root_envelope_renders_absolute_service_urls() {
    let base = BaseUrl::parse("https://packages.example/prefix/").unwrap();
    let document = root_envelope(Some(&base), vec![json!({"name": "packages"})]);

    assert_eq!(document["urls"]["api"], "https://packages.example/prefix/+api");
    assert_eq!(document["urls"]["health"], "https://packages.example/prefix/+health");
    assert_eq!(document["urls"]["readiness"], "https://packages.example/prefix/+ready");
    assert_eq!(document["urls"]["status"], "https://packages.example/prefix/+status");
    assert_eq!(document["urls"]["stats"], "https://packages.example/prefix/+stats");
    assert_eq!(
        document["urls"]["openapi"],
        "https://packages.example/prefix/api-docs/openapi.json"
    );
    assert_eq!(document["urls"]["web"], "https://packages.example/prefix/");
    assert_eq!(document["indexes"], json!([{"name": "packages"}]));
}
...
fn test_request_uri_origin_precedes_forwarded_headers() {
    ...
    let uri = Uri::from_static("https://packages.example/+api");
