...
use crate::serving::{DriverCapabilities, EcosystemDriver, RouteMount, ServiceDriver};
...
async fn test_upstream_limit_times_out_with_retry_horizon() {
    ...
    let waiting = {
        ...
        tokio::spawn(async move { limits.acquire("bounded").await })
    };
...
fn app(config: RateLimitConfig) -> (tempfile::TempDir, Arc<AppState>) {
    let dir = tempfile::tempdir().unwrap();
    let meta = MetaStore::open(dir.path().join("peryx.redb")).unwrap();
    let blobs = BlobStore::new(dir.path().join("blobs"));
    (
        dir,
        Arc::new(AppState::with_rate_limits(meta, blobs, 60, Vec::new(), config, [])),
    )
}
...
async fn test_enforce_uses_the_indexed_drivers_route_class() {
    ...
    state.register_ecosystem(Arc::new(IndexedDriver), Arc::new(EmptyIndexer));
    let router = router(Arc::new(state));

    let request = || {
        Request::get("/packages/item")
...
async fn test_enforce_uses_an_ecosystem_service_post_class() {
    ...
    Arc::get_mut(&mut state)
        .unwrap()
        .register_ecosystem(Arc::new(IndexedDriver), Arc::new(EmptyIndexer));
    let router = router(state);
...
async fn test_enforce_uses_an_absolute_drivers_route_class() {
    ...
    Arc::get_mut(&mut state)
        .unwrap()
        .register_ecosystem(Arc::new(AbsoluteDriver), Arc::new(EmptyIndexer));
    let router = router(state);
