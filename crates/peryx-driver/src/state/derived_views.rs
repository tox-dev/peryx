...
//!
//! The replica loop exports the frontier as `peryx_ha_distributed_readable_serial`. Each ecosystem
//! holds a response until every required view
...
/// [`apply_replicated_changes`](crate::serving::ReplicatedApplyDriver::apply_replicated_changes) so the neutral
...
/// Returns the minimum readable serial and the required view holding it back.
///
/// An absent view has applied nothing and pins readability to zero. Otherwise, `authority` and all
/// required view frontiers bound the result.
#[must_use]
...
#[cfg(test)]
#[path = "../../tests/unit/state/derived_views/tests.rs"]
mod tests;
