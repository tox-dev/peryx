...
impl TooSlow {
    #[must_use]
    pub const fn new(shortfall: u64) -> Self {
        Self(shortfall)
    }
}
...
pub enum BodyFailure {
    ...
    Stalled(Duration),
    /// The client kept sending without keeping up with the floor, so the request never completed
    /// either. Repeating it over a link that can sustain the transfer is what finishes it.
    TooSlow(u64),
...
impl BodyFailure {
    ...
    pub fn of(error: &(dyn std::error::Error + 'static)) -> Self {
        ...
        while let Some(error) = current {
            ...
            if let Some(slow) = error.downcast_ref::<TooSlow>() {
                return Self::TooSlow(slow.0);
            }
