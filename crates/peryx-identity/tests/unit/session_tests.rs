...
fn test_a_session_key_does_not_open_a_pre_auth_cookie() {
    // Domain separation: the two cookies derive independent keys from the same secret, so a value cookie
    // for one purpose fails to authenticate under the other's key.
    let sealer = sealer();
