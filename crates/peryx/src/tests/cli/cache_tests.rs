...
fn test_parse_cache_size_and_fsck() {
    let size = parse(&["peryx", "cache", "size", "--data-dir", "/d"]);
    let Command::Cache(CacheCommand::Size(args)) = size.command else {
        panic!("expected cache size");
    };
    assert_eq!(args.runtime.data_dir, Some(PathBuf::from("/d")));

    let fsck = parse(&["peryx", "cache", "fsck", "--data-dir", "/d"]);
...
#[case::purge_project(
    &["peryx", "cache", "purge", "project", "--data-dir", "/project", "--index", "pypi", "--project", "Flask"][..],
    "/project"
)]
