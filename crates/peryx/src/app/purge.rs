...
pub(super) fn purge_orphaned_blobs(
    ...
    let report = peryx_ha_distributed::purge_orphaned_blobs(&stores.meta, &stores.blobs, args.yes, now, || {
        referenced_blob_digests_with_drivers(drivers, &stores.meta).map_err(|error| format!("{error:#}"))
    })?;
    write_orphan_purge_report(out, if args.yes { "removed" } else { "dry-run" }, &report)
}

fn write_orphan_purge_report(out: &mut dyn Write, action: &str, report: &OrphanPurgeReport) -> anyhow::Result<()> {
...
pub fn referenced_blob_digests_with_drivers(drivers: &DriverSet, meta: &MetaStore) -> anyhow::Result<BTreeSet<String>> {
    let mut digests = BTreeSet::new();
    for driver in drivers.blob_reference_drivers() {
