...
use super::*;
use crate::app::tests::bounded_output;
use crate::cli::{
...
#[test]
fn test_revocation_put_sends_json_with_basic_authentication() {
...
fn test_revocation_list_sends_stable_query_parameters(
...
fn test_revocation_targets_digest_operations(
    ...
    client.server = format!("{}/admin?discarded=yes#fragment", server.uri());
    let command = if lift {
...
#[test]
fn test_revocation_reads_password_file() {
    let (runtime, server) = runtime_and_server();
    runtime.block_on(
        Mock::given(header(
            "authorization",
...
#[test]
fn test_revocation_reports_a_missing_password_file() {
    let (_runtime, server) = runtime_and_server();
    let command = inspect(AdministratorClientArgs {
        password_stdin: false,
        password_file: Some(PathBuf::from("missing-administrator-password")),
        ..client(&server)
    });

    let error = revocation(&command, &mut Cursor::new(Vec::new()), &mut Vec::new()).unwrap_err();

    assert!(error.to_string().contains("open password file"), "{error:#}");
}
...
fn test_revocation_rejects_unsafe_server_urls(#[case] server: &str, #[case] expected: &str) {
    ...
    let error = revocation(&command, &mut Cursor::new(PASSWORD), &mut Vec::new()).unwrap_err();
...
fn test_revocation_rejects_failed_or_unbounded_responses(#[case] failure: ResponseFailure, #[case] expected: &str) {
    let (runtime, server) = runtime_and_server();
    let response = match failure {
        ...
        ResponseFailure::Oversized => ResponseTemplate::new(200).set_body_bytes(vec![b'a'; MAX_RESPONSE_BYTES + 1]),
    };
...
fn test_revocation_validates_digest_inputs(#[case] digest: &str, #[case] cursor: Option<&str>, #[case] expected: &str) {
    let (_runtime, server) = runtime_and_server();
...
#[test]
fn test_revocation_propagates_output_failures() {
    let (runtime, server) = runtime_and_server();
    runtime.block_on(
