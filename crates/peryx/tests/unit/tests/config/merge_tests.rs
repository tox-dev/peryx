...
fn test_acme_staging_and_cache_dir() {
    ...
    assert_eq!(
        ...
        Some(TlsConfig::Acme(AcmeConfig {
            ...
            cache_dir: PathBuf::from("/var/acme"),
            staging: true,
        }))
    );
}
...
fn test_apply_overlays_only_present_fields() {
    ...
    assert_eq!(merged.indexes.len(), 6);
}
...
[[index]]\nname = \"secret\"\nhosted = true\n\
[[index]]\nname = \"team\"\nroute = \"team/dev\"\nlayers = [\"team-hosted\", \"public\"]\nwrite_target = \"team-hosted\"\n";
    let c = toml_config(text);
    assert_eq!(c.indexes.len(), 5);
    assert_eq!(c.indexes[0].route, "public"); // route defaults to name
    assert!(matches!(
        &c.indexes[0].kind,
    ...
    assert!(
        matches!(&c.indexes[4].kind, IndexKind::Virtual { layers, write_target: Some(write_target) }
            if layers == &["team-hosted".to_owned(), "public".to_owned()] && write_target == "team-hosted")
    );
}
...
"#,
    ...
    assert!(matches!(
        ...
        IndexKind::Cached { routing, .. } if {
            ...
            (
                ...
                Some(Path::new("/run/tls/single-ca.pem")),
    ...
    assert!(matches!(
        ...
        IndexKind::Cached { routing, .. } if {
            let tls = &routing.upstreams[0].tls;
            tls.ca_file.as_deref() == Some(Path::new("/run/tls/primary-ca.pem"))
                && format!("{tls:?}")
...
#[rstest]
#[case::cached("cached = \"https://upstream.example/api/\"", "cached")]
...
fn test_upstream_credential_refresh_resolves_for_file_sources() {
    ...
    assert!(matches!(
        ...
        IndexKind::Cached { routing, .. }
            if matches!(
                routing.upstreams[0].credential_refresh,
                Some(refresh)
                    if (refresh.interval, refresh.on_unauthorized, refresh.failure)
    ...
    assert!(matches!(
        ...
        IndexKind::Cached { routing, .. }
            if matches!(
                routing.upstreams[0].credential_refresh,
                Some(refresh)
                    if (refresh.on_unauthorized, refresh.failure)
...
fn test_exec_credential_resolves_for_a_cached_index() {
    ...
    assert!(matches!(
        ...
        IndexKind::Cached { routing, .. }
            if matches!(
                &routing.upstreams[0].credential_exec,
                Some(exec)
                    if (exec.argv(), exec.timeout(), exec.environment(), exec.failure())
                        == (
    ...
    assert!(matches!(
        ...
        IndexKind::Cached { routing, .. }
            if matches!(
                &routing.upstreams[0].credential_exec,
                Some(exec)
                    if (exec.timeout(), exec.environment(), exec.failure())
...
#[test]
fn test_write_target_requires_layers() {
    assert!(
        toml_error("[[index]]\nname = \"hosted\"\nhosted = true\nwrite_target = \"other\"\n")
            .to_string()
            .contains("`write_target` requires `layers`")
    );
}
