...
fn cached_config(data_dir: &Path, upstream: String, files: &TlsFiles) -> Config {
    Config {
        data_dir: data_dir.to_path_buf(),
        indexes: vec![IndexConfig {
            name: "private".to_owned(),
            route: "private".to_owned(),
            ecosystem: crate::compiled_plugins().default_ecosystem(),
...
#[tokio::test]
async fn test_build_state_uses_upstream_ca_and_client_identity(#[case] request: RequestKind) {
    let pki = TestPki::new();
    let files = pki.write_client_files();
    let server = pki.start_server(ok_router());
    let data = tempfile::tempdir().unwrap();
    let state = build_state(&cached_config(data.path(), server.url("/"), &files)).unwrap();

    assert!(
        request
            .succeeds(state.serving.indexes[0].proxy_client().unwrap(), &server.url("/ok"),)
            .await
    );
}
...
fn test_build_state_rejects_unreadable_tls_before_serving() {
    ...
    let data = tempfile::tempdir().unwrap();
    let error = build_state(&cached_config(
        data.path(),
        "https://upstream.example/".to_owned(),
        &files,
    ))
    .err()
    .expect("invalid upstream TLS must fail state construction");
...
fn test_build_state_rejects_incomplete_programmatic_tls_identity() {
    ...
    let files = pki.write_client_files();
    let data = tempfile::tempdir().unwrap();
    let mut config = cached_config(data.path(), "https://upstream.example/".to_owned(), &files);
    if let IndexKind::Cached { routing, .. } = &mut config.indexes[0].kind {
        routing.upstreams[0].tls.client_key_file = None;
    }
    let error = build_state(&config)
        .err()
        .expect("incomplete upstream TLS identity must fail state construction");
...
async fn test_rebuilding_state_reloads_rotated_tls_files() {
    ...
    let data = tempfile::tempdir().unwrap();
    let config = cached_config(data.path(), server.url("/"), &files);
    let previous_state = build_state(&config).unwrap();
    assert!(
        previous_state.serving.indexes[0]
            .proxy_client()
            .unwrap()
            .fetch_bytes(&server.url("/ok"))
            .await
    ...
    assert_eq!(
        replacement_state.serving.indexes[0]
            .proxy_client()
            .unwrap()
            .fetch_bytes(&server.url("/ok"))
            .await
