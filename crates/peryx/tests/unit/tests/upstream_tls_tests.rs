...
#[rstest]
#[case::pypi_artifact(peryx_ecosystem_pypi::ECOSYSTEM, RequestKind::Artifact)]
#[case::pypi_metadata(peryx_ecosystem_pypi::ECOSYSTEM, RequestKind::Metadata)]
#[case::oci_artifact(peryx_ecosystem_oci::ECOSYSTEM, RequestKind::Artifact)]
#[case::oci_metadata(peryx_ecosystem_oci::ECOSYSTEM, RequestKind::Metadata)]
#[tokio::test]
...
async fn test_rebuilding_state_reloads_rotated_tls_files() {
    ...
    let data = tempfile::tempdir().unwrap();
    let config = cached_config(data.path(), peryx_ecosystem_pypi::ECOSYSTEM, server.url("/"), &files);
