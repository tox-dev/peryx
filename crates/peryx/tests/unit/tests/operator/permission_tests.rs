...
/// Pin the process file-creation mask to a loose `022` for a permission test and restore the caller's
/// mask on drop. The lock serializes these tests so the shared, process-global mask one test sets is
