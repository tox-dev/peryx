...
fn test_netrc_resolves_origin_before_authority_and_host() {
    ...
    assert_eq!(
        netrc.auth_for_str("https://example.com/api/").unwrap(),
        basic("origin", "origin-pass")
    );
    assert_eq!(
        netrc.auth_for_str("http://example.com:443/api/").unwrap(),
        basic("authority", "authority-pass")
    );
    assert_eq!(netrc.auth_for_str("https://example.com:8443/api/").unwrap(), Auth::None);
}
...
fn test_netrc_bare_host_matches_a_default_port() {
    ...
    assert_eq!(
        netrc.auth_for_str("https://plain.example/api/").unwrap(),
        basic("reader", "secret")
    );
}
...
fn test_netrc_resolves_default_and_ignores_empty_default() {
    ...
    assert_eq!(
        netrc.auth_for_str("https://missing.example/api/").unwrap(),
        basic("fallback", "fallback-pass")
    );
    let (_dir, netrc) = load("default\n");
    assert_eq!(netrc.auth_for_str("https://missing.example/api/").unwrap(), Auth::None);
}
...
fn test_netrc_matches_ipv4_but_not_schemes_without_known_ports() {
    ...
    assert_eq!(
        netrc.auth_for_str("https://192.0.2.1/api/").unwrap(),
        basic("ipv4", "address")
    );
    assert_eq!(netrc.auth_for_str("custom://example.com/api/").unwrap(), Auth::None);
}

#[test]
fn test_netrc_returns_anonymous_for_urls_without_hosts() {
    let (_dir, netrc) = load("default login fallback password fallback-pass\n");
    assert_eq!(netrc.auth_for_str("file:///tmp/artifact.bin").unwrap(), Auth::None);
...
fn test_netrc_rejects_a_file_owned_by_another_user() {
    ...
    .is_ok();
    let paths = [std::path::PathBuf::from("/etc/hosts"), path];

    assert!(
        Netrc::from_path(&paths[usize::from(owner_changed)])
            .unwrap_err()
            .to_string()
