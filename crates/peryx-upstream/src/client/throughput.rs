...
pub const THROUGHPUT_FLOOR: u64 = 64 * 1024;
...
impl<Body> Bounded<Body> {
    ...
    fn is_starved(&mut self, now: tokio::time::Instant) -> bool {
        ...
        let nanos = u128::from(self.delivered) * 1_000_000_000 / u128::from(THROUGHPUT_FLOOR);
        let earned = THROUGHPUT_GRACE.saturating_add(Duration::from_nanos(u64::try_from(nanos).unwrap_or(u64::MAX)));
        now.saturating_duration_since(started) > earned
    }
}
