...
impl Fixture {
    ...
    async fn with_fault(fault: StoreFault) -> Self {
        ...
        let mut state = AppState::with_clock(meta.clone(), blobs, 60, Vec::new(), Arc::new(|| 42));
        state.users = users;
    ...
    fn seed_run(&self, finish: bool) -> String {
        let id = self
            ...
            .unwrap();
        if finish {
            self.meta.finish_job_run(&id, JobOutcome::succeeded(2, 0, 0)).unwrap();
        }
        id
    }
}
...
async fn test_cancel_signals_a_running_attempt_and_it_unwinds() {
    ...
    assert_eq!(
        fixture.meta.get_job_run(&running.id).unwrap().unwrap().state,
        JobState::Cancelled
    );
}
