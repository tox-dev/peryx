...
use peryx_identity::{
    ExternalIdentity, ExternalLinkRequest, ExternalSubject, OidcLoginProvider, OidcLoginService, OidcProviderSettings,
    PRE_AUTH_COOKIE, PendingLogin, ProviderId, SESSION_COOKIE, ServerUser, SessionSealer, UserId, UserName, UserState,
};
...
fn settings(issuer: &str) -> OidcProviderSettings {
    OidcProviderSettings {
        ...
        client_secret: None,
        redirect_uri: url::Url::parse("http://registry.example/_/login/corporate/callback").unwrap(),
...
/// A state that serves one OIDC provider pointed at `issuer`, with the session sealer configured.
fn state_with_provider(issuer: &str) -> (tempfile::TempDir, Arc<AppState>) {
    let dir = tempfile::tempdir().unwrap();
    ...
    let provider = OidcLoginProvider::insecure(settings(issuer)).unwrap();
    state.set_session_sealer(SessionSealer::new(KEY));
    state.set_oidc_logins([OidcLoginService::new(provider, meta, Vec::new())]);
    (dir, Arc::new(state))
}
...
async fn test_login_start_without_a_configured_sealer_is_a_server_error() {
    // A provider without a session sealer is a misconfiguration the config layer forbids, but the
    // handler still fails closed rather than panicking.
    let dir = tempfile::tempdir().unwrap();
    let meta = peryx_storage::meta::MetaStore::open(dir.path().join("peryx.redb")).unwrap();
    let mut state = AppState::new(
        meta.clone(),
    ...
    let provider = OidcLoginProvider::insecure(settings("http://issuer.invalid/")).unwrap();
    state.set_oidc_logins([OidcLoginService::new(provider, meta, Vec::new())]);
...
async fn test_login_start_maps_an_unreachable_provider_to_service_unavailable() {
    // `authorization` fetches discovery from a dead issuer; the transport failure surfaces as a
    // retryable 503, exercising login-start's provider-error arm.
...
async fn test_callback_without_a_configured_sealer_is_a_server_error() {
    // A provider configured without a session sealer is a misconfiguration; the callback fails closed
    // with 500 rather than panicking.
    let dir = tempfile::tempdir().unwrap();
    let meta = peryx_storage::meta::MetaStore::open(dir.path().join("peryx.redb")).unwrap();
    let mut state = AppState::new(
        meta.clone(),
...
async fn test_a_disabled_linked_user_surfaces_a_store_error() {
    // The identity is already linked to a disabled user, so a valid token still fails to resolve: the
    // linking store returns its error and the callback maps it to a 500 rather than minting a session.
    ...
    let cookie = format!(
        "{PRE_AUTH_COOKIE}={}",
        SessionSealer::new(KEY).seal_pre_auth(&authorization.pending, now() + 600)
    );
    let uri = format!(
...
async fn test_callback_with_a_mismatched_state_fails_authentication() {
    // The handoff seals state "expected"; the provider returns "forged". The engine rejects the state
    // before any token exchange, so the handler answers 401 without reaching the network.
    let (_dir, state) = state_with_provider("http://issuer.invalid/");
    ...
    let cookie = format!(
        "{PRE_AUTH_COOKIE}={}",
        SessionSealer::new(KEY).seal_pre_auth(&pending, now() + 600)
    );
    let response = send(
        state,
        Method::GET,
...
async fn test_callback_maps_an_unreachable_provider_to_service_unavailable() {
    // The state matches, so the engine proceeds to fetch discovery from a dead issuer; a transport
    // failure surfaces as a retryable 503, not a rejected login.
    let (_dir, state) = state_with_provider("http://127.0.0.1:1/");
    let pending = PendingLogin {
        state: "s".to_owned(),
        nonce: "n".to_owned(),
...
async fn test_session_without_a_configured_sealer_reports_no_user() {
    let (_dir, state) = empty_state();
    let sealed = SessionSealer::new(KEY).seal_session(&user(), now() + 3600);
...
async fn test_session_with_an_expired_cookie_reports_no_user() {
    ...
    state.set_session_sealer(SessionSealer::new(KEY));
    let _keep = dir;
    let sealed = SessionSealer::new(KEY).seal_session(&user(), now() - 1);
    let cookie = format!("{SESSION_COOKIE}={sealed}");
    let response = send(Arc::new(state), Method::GET, "/_/session", Some(&cookie)).await;
    assert_eq!(body_json(response).await["user"], Value::Null);
}
...
async fn test_login_start_redirects_to_the_provider_and_seals_the_handoff() {
    let server = MockServer::start().await;
    mount_issuer(&server).await;
    let (_dir, state) = state_with_provider(&format!("{}/", server.uri()));

    let response = send(state, Method::GET, "/_/login/corporate", None).await;

    assert_eq!(response.status(), StatusCode::SEE_OTHER);
    assert!(
        location(&response).starts_with(&format!("{}/authorize", server.uri())),
        "{}",
        location(&response)
    );
...
async fn test_a_valid_callback_creates_a_session() {
    ...
    // Mint the pending handoff through the engine so its state and nonce are the ones the provider bound,
    // then answer the token endpoint with an ID token carrying that nonce.
    let authorization = provider.authorization(now()).await.unwrap();
    let id_token = mint(&issuer, &authorization.pending.nonce);
    Mock::given(method("POST"))
        .and(path("/token"))
