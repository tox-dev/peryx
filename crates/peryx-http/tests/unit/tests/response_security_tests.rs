...
#[case::repository_reader(
    ...
    serde_json::json!({"artifact": "private-artifact", "version": "0.0.1"})
)]
#[case::operator(
...
fn test_filter_fields_accepts_an_authorized_repository_token() {
    assert_eq!(
        ...
        serde_json::from_value(serde_json::json!({"artifact": "private-artifact", "version": "0.0.1"})).unwrap()
    );
}
...
fn fixture() -> Vec<ClassifiedField> {
    vec![
        ...
        ClassifiedField::new(
            ...
            serde_json::json!("private-artifact"),
        ),
