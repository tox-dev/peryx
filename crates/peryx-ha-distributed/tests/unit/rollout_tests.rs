...
/// A membership every committed voter supports the `target(2, 2)` on.
fn membership() -> Vec<AvailabilityVersions> {
    vec![versions((1, 3), (1, 3)), versions((1, 3), (1, 3))]
}

/// A caught-up two-member group: writer and replica both at serial 10.
...
#[test]
fn test_verdict_types_render_for_debug() {
    assert!(format!("{:?}", budget(1, 0, 0)).contains("RolloutBudget"));
    assert!(format!("{:?}", Version(TargetUnsupported)).contains("Version"));
    assert!(format!("{:?}", RolloutPreflight::Ready).contains("Ready"));
}
