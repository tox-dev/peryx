...
use crate::visibility_feed::{
    ApplyEnvelopeError, OpenError, VisibilityFeedError, VisibilityProjection, decode_visibility_op, visibility_change,
    visibility_envelope,
};
...
fn test_visibility_change_carries_the_operation_serial_without_row_or_blob_changes() {
    let change = visibility_change(&op("root/alpha/resource-a/artifact-a", Trash, 3, 9));
...
fn test_visibility_envelope_tags_the_kind_and_epoch_and_round_trips_the_operation() {
    let minted = op("root/alpha/resource-a/artifact-a", Revoke, 4, 12);
...
fn test_decode_ignores_an_envelope_of_another_kind() {
    let change = visibility_change(&op("root/alpha/resource-a/artifact-a", Trash, 1, 1));
...
fn test_decode_rejects_a_payload_schema_this_build_does_not_apply() {
    let event = serde_json::to_vec(&serde_json::json!({
        "schema": 2,
        "artifact": {"coordinate": "root/alpha/resource-a/artifact-a", "digest": "sha256:deadbeef"},
        "action": "trash",
...
fn test_decode_rejects_a_serial_that_disagrees_with_the_envelope() {
    let mut change = visibility_change(&op("root/alpha/resource-a/artifact-a", Trash, 7, 7));
    change.serial = 5;
...
fn test_decode_rejects_an_epoch_that_disagrees_with_the_envelope() {
    let change = visibility_change(&op("root/alpha/resource-a/artifact-a", Trash, 9, 5));
...
fn test_open_on_an_empty_store_serves_the_visible_default() {
    ...
    assert!(
        projection
            .visibility(&artifact("root/alpha/resource-a/artifact-a"))
            .is_visible()
    );
