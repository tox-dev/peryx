...
async fn build_app(corrupt: bool) -> (tempfile::TempDir, Arc<AppState>) {
    ...
    drop(authorization);
    drop(users);
    crate::record_artifact_placement(&meta, "sha256:aaa", ArtifactSource::Hosted, true).unwrap();
    crate::record_artifact_placement(&meta, "sha256:bbb", ArtifactSource::Proxy, false).unwrap();
    crate::record_artifact_placement(&meta, "sha256:ccc", ArtifactSource::Generated, false).unwrap();
    drop(meta);
    if corrupt {
    ...
    super::support::install_distributed_services(&mut state);
    (dir, Arc::new(state))
}
