...
fn batch(producer: &str, epoch: u64, day: i64, rows: &[(&str, &str, u64, u64)]) -> AnalyticsBatch {
    AnalyticsBatch {
        ...
        rows: rows
            .iter()
            .map(|(repository, resource, downloads, bytes)| AggregateRow {
                key: AggregateKey {
                    day,
...
fn test_every_producer_at_the_frontier_is_complete() {
    let batches = [
        ...
        batch("east", 1, 10, &[("alpha", "resource-a", 3, 30)]),
        batch("west", 1, 10, &[("alpha", "resource-b", 5, 50)]),
    ];
...
fn test_a_trailing_producer_is_delayed() {
    let batches = [
        batch("east", 1, 10, &[("alpha", "resource-a", 3, 30)]),
        batch("west", 1, 8, &[("alpha", "resource-b", 5, 50)]),
    ];
    let report = assess_completeness(
        &receiver(&batches),
        &expected(&[("east", "east-dc"), ("west", "west-dc")]),
...
fn test_an_empty_expected_set_is_unavailable() {
    let report = assess_completeness(
        &receiver(&[batch("east", 1, 10, &[("alpha", "resource-a", 3, 30)])]),
        &expected(&[]),
        &query(0, 10, 11, None),
    );
...
fn test_a_historical_range_below_the_frontier_covers_a_laggard() {
    let batches = [
        batch("east", 1, 20, &[("alpha", "resource-a", 3, 30)]),
        batch("west", 1, 10, &[("alpha", "resource-b", 5, 50)]),
    ];
    let report = assess_completeness(
        &receiver(&batches),
...
fn test_repository_scope_confines_the_totals() {
    let batches = [batch(
        ...
        1,
        10,
        &[("alpha", "resource-a", 3, 30), ("other", "resource-b", 5, 50)],
    )];
    let report = assess_completeness(
        &receiver(&batches),
        &expected(&[("east", "east-dc")]),
...
fn test_the_day_range_bounds_the_buckets() {
    let batches = [
        batch("east", 1, 5, &[("alpha", "resource-a", 1, 10)]),
        batch("east", 1, 10, &[("alpha", "resource-a", 2, 20)]),
        batch("east", 1, 15, &[("alpha", "resource-a", 4, 40)]),
    ];
    let report = assess_completeness(
        &receiver(&batches),
        &expected(&[("east", "east-dc")]),
...
fn test_a_restart_under_a_higher_epoch_leads_the_frontier() {
    let batches = [
        batch("east", 1, 10, &[("alpha", "resource-a", 3, 30)]),
        batch("east", 2, 4, &[("alpha", "resource-a", 1, 10)]),
    ];
    let report = assess_completeness(
        &receiver(&batches),
        &expected(&[("east", "east-dc")]),
...
fn test_an_unexpected_producer_is_left_out_of_the_totals() {
    let batches = [
        batch("east", 1, 10, &[("alpha", "resource-a", 10, 100)]),
        batch("rogue", 1, 10, &[("alpha", "resource-a", 1_000, 10_000)]),
    ];
    let report = assess_completeness(
        &receiver(&batches),
        &expected(&[("east", "east-dc")]),
...
fn test_a_duplicate_delivery_folds_the_totals_once() {
    let delivery = batch("east", 1, 10, &[("alpha", "resource-a", 3, 30)]);
    let mut receiver = AnalyticsReceiver::new(DEFAULT_APPLY_LIMITS);
...
fn test_distributed_reader_restores_the_durable_receiver() {
    ...
    meta.analytics()
        .save_apply(&receiver(&[batch("east", 1, 10, &[("alpha", "resource-a", 3, 30)])]).encode())
        .unwrap();
