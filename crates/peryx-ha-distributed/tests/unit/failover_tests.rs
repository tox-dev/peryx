...
fn test_a_suspect_home_holds_because_suspicion_never_moves_authority() {
    // A suspect home is a routing hint, not a failure, so a transfer target being ready changes nothing.
...
fn test_a_dead_home_takes_the_first_alive_candidate_in_order() {
    // A suspect candidate is skipped for the alive one behind it, and order is preserved so the choice
    // is deterministic across evaluations.
    let candidates = [
...
fn test_evaluation_is_bounded_and_ignores_a_candidate_past_the_limit() {
    // The one alive candidate sits past the evaluation bound, so a bounded pass does not reach it and
    // reports no candidate rather than scanning an unbounded roster.
    let candidates = [
