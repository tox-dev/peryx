...
async fn test_different_digests_route_to_different_datacenters() {
    // Each datacenter holds ONLY its own blob, so a request that reaches the wrong delegate misses. An
    // impl that routes every digest to one shared transport fails the second fetch.
