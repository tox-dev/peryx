...
fn test_revoke_then_lift_toggles_visibility() {
    let mut state = VisibilityState::new();
    let art = artifact("root/beta/resource-b/artifact-b");
...
fn test_encode_then_restore_reproduces_the_converged_visibility() {
    let mut state = VisibilityState::new();
    let trashed = artifact("root/alpha/resource-a/artifact-a");
    let revoked = artifact("root/beta/resource-b/artifact-b");
    let visible = artifact("root/alpha/resource-c/artifact-c");
