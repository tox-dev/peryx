...
/// A transport that answers every request with one preset frame, so a test can feed the driver a
/// non-contiguous or empty page the well-formed `LoopbackTransport` never produces.
