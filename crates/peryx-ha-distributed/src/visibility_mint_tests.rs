...
#[test]
fn test_stale_epoch_is_debug() {
    let error = StaleEpoch {
        current: 5,
        presented: 4,
    };
    assert!(format!("{error:?}").contains("StaleEpoch"));
}
...
#[test]
fn test_minter_is_debug() {
    assert!(format!("{:?}", minter(1)).contains("VisibilityMinter"));
}
...
fn test_journal_serials_draw_strictly_increasing_serials_across_a_reopen() {
    ...
    assert!(format!("{serials:?}").contains("JournalSerials"));
