...
async fn test_gather_returns_live_without_a_query_when_already_durable() {
    ...
    // A source that would fault if consulted; a met durability must return before touching it.
...
async fn test_gather_never_counts_a_remote_at_a_stale_epoch() {
    // The remote applied the frontier but under an older authority epoch, so it is fenced out.
...
async fn test_gather_keeps_only_a_remotes_latest_report() {
    // A remote reporting behind the frontier every round is re-recorded, not accumulated: its single
    // entry stays at the latest value and never satisfies the operation.
