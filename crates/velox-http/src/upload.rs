...
use velox_core::pypi::{CoreMetadata, File, Yanked, normalize_name};
...
/// The fields velox reads from an upload's multipart form. Every field is optional here so the
...
/// sha256, with its download URL pointing at velox's own file route on `index`.
///
/// # Errors
