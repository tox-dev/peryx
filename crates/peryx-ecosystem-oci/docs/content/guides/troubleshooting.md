...
These responses apply when configuration selects `availability.mode = "dc"` or `"ha"`. A read-only replica refuses blob
and manifest mutations with `503 Service Unavailable`. A request under a superseded repository epoch returns `503` with
code `UNAVAILABLE`. Retry the same operation against the current writer. Resumable blob uploads retain their session and
staged bytes across this refusal.

See [Availability behavior](../reference/availability.md).
...
See [OCI policy settings](../reference/policy.md#push-quotas) and [Token authentication](../reference/token-auth.md).
