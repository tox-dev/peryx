...
description = "Proxy non-sha256 manifests and cancel or resume OCI blob uploads."
weight = 5
aliases = [ "/ecosystems/oci/guides/non-sha256-registry/", "/ecosystems/oci/guides/cancel-and-resume-push/"]
+++

The examples proxy an upstream that advertises non-sha256 manifest digests and recover an interrupted blob upload. They
use peryx at `http://127.0.0.1:4433`.
...
- **A wrong sha256 advertisement.** If the upstream advertises a `sha256:` digest that does not hash the bytes it sent,
  peryx returns `502` and caches nothing.
- **Offline mirror pins.** A [mirror](air-gapped.md) entry pinned by digest must be `sha256:`. `repo@sha512:…` fails the
  mirror's own sha256 comparison; mirror by tag instead, which stores under the canonical sha256.
...
A `200` with `docker-content-digest: sha256:...` confirms the proxy response. A `502` means the upstream advertised a
`sha256:` digest that did not match its bytes. The exact rules are in
[content digest algorithms](../reference/registry-behavior.md#content-digest-algorithms), and the reasoning in
[why peryx accepts a non-sha256 content digest](../registry-behavior.md#digest-handling).
...
The collector asks each registered blob-reference provider for references and retains a digest while any provider
reports it.
...
peryx answer `416`. Both cleanups act on an [upload session](../reference/registry-behavior.md#upload-sessions), so both
need the hosted index's access-token secret as the Basic-auth password (`-u _:<token>`).
...
[push a blob chunk by chunk](../tutorials/chunked-upload.md).

## Related

- The statuses, headers, and digest rules these commands rely on: [registry behavior](../reference/registry-behavior.md)
- Registry behavior rationale: [registry behavior](../registry-behavior.md)
- `/v2/` upload verbs and success codes: [HTTP endpoints](../reference/endpoints.md) </content>
