...
referrers subject digest. See [HTTP endpoints](endpoints.md), [standards](standards.md), and
[registry behavior decisions](../registry-behavior.md).
...
- The offline mirror still pins a by-digest reference to sha256. A [mirror](../guides/air-gapped.md) entry written as
  `repo@sha512:…` fails, because the mirror compares the reference against the sha256 it computes. The relaxation is on
  the online pull-through path, not the mirror pin.
