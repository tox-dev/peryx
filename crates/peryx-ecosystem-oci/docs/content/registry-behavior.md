...
- [Registry behavior reference](reference/registry-behavior.md) lists exact headers, status codes, and digest rules.
- [Work with registry behavior](guides/registry-behavior.md) contains cancellation, resume, and reclamation commands.
- [Chunked upload tutorial](tutorials/chunked-upload.md) exercises the upload protocol with `curl`.
- [Availability behavior](reference/availability.md) covers routing and retry behavior for `dc` and `ha`.
