...
fn test_chunk_range_rejects_bytes_that_are_not_text() {
    // Protocol fields with invalid text encoding are malformed.
...
fn test_chunk_range_rejects_a_span_that_overflows() {
    // An overflowing inclusive range has no representable length.
...
fn test_range_refuses_a_length_the_body_does_not_carry() {
    ...
    // Unknown body lengths cannot validate a declared range.
