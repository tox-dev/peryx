//! A proxied `tags/list` body names the repository the client addressed, not the upstream one, on
//! every path that answers it: direct, cached, paginated, library-prefixed, and virtual.
...
async fn test_cached_tag_list_serves_the_client_name_without_upstream() {
    ...
    // Prime the cache, then drop the upstream: the fresh cache still answers with the client name.
