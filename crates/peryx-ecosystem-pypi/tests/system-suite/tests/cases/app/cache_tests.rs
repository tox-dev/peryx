...
fn test_cache_list_reports_write_errors() {
    ...
    let err = app::cache(&config, &cache_list_command(), &mut bounded_output(0)).unwrap_err();
    assert!(err.to_string().contains("failed to write whole buffer"));
}

#[test]
fn test_cache_list_reports_index_page_write_errors() {
    let (_dir, config, _digest) = cache_fixture();
    let mut out = bounded_output(CACHE_LIST_HEADER.len());
...
fn test_cache_rejects_the_s3_blob_backend() {
    ...
    let config = Config {
        ...
        blob: crate::support::s3_blob_backend(),
        ..Config::default()
    };
...
const fn cache_list_command() -> CacheCommand {
    CacheCommand::List(CacheListArgs {
        runtime: runtime_args(),
        index: None,
        resource: None,
...
const fn size_command() -> CacheCommand {
