...
use super::{blob_relpath, bounded_through_line, claimed_data_dir, identified_backup, valid_backup};
...
fn test_restore_propagates_a_rollback_warning_write_error() {
    let (_holder, backup) = identified_backup("node-a", 1);
    let probe = claimed_data_dir("node-a", 3);
    let mut complete = Vec::new();
    operator::restore(&backup, probe.path(), true, &mut complete).unwrap();
    let target = claimed_data_dir("node-a", 3);
    let mut out = bounded_through_line(&complete, "warning\trestore\trollback");

    operator::restore(&backup, target.path(), true, &mut out).unwrap_err();

    let seen = String::from_utf8(out.into_inner().into_vec()).unwrap();
    assert!(seen.contains("warning\trestore\trollback"), "{seen}");
    let meta = MetaStore::open_existing_read_only(target.path().join("peryx.redb")).unwrap();
...
#[test]
fn test_restore_rejects_an_unreadable_target_store_without_replacing_it() {
    let (_holder, backup) = identified_backup("node-a", 1);
    let target = tempfile::tempdir().unwrap();
    std::fs::write(target.path().join("peryx.redb"), b"not a redb database").unwrap();

    let error = operator::restore(&backup, target.path(), true, &mut Vec::new()).unwrap_err();

    assert!(error.to_string().contains("open metadata store"));
    assert_eq!(
