...
pub(super) fn md5_hex(bytes: &[u8]) -> String {
    hex(&<Md5 as md5::Digest>::digest(bytes))
}
...
pub(super) const fn wheel_record_entries() -> [(&'static str, &'static [u8]); 3] {
    [
        ("Flask/__init__.py", b"VALUE = 1\n".as_slice()),
        (
...
pub(super) fn record_line(path: &str, bytes: &[u8], size: usize) -> String {
    record_line_with_algorithm(path, bytes, size, RecordHash::Sha256)
}

pub(super) fn record_with_self_size(entries: &[(&str, &[u8], RecordHash)], record_path: &str) -> String {
    let mut record = String::new();
...
pub(super) fn record_line_with_algorithm(path: &str, bytes: &[u8], size: usize, algorithm: RecordHash) -> String {
    let digest = match algorithm {
        RecordHash::Sha256 => URL_SAFE_NO_PAD.encode(Sha256::digest(bytes)),
        RecordHash::Sha384 => URL_SAFE_NO_PAD.encode(Sha384::digest(bytes)),
        RecordHash::Sha512 => URL_SAFE_NO_PAD.encode(Sha512::digest(bytes)),
    };
    format!("{path},{}={digest},{size}\n", algorithm.name())
}

#[derive(Clone, Copy)]
pub(super) enum RecordHash {
    Sha256,
    Sha384,
    Sha512,
}
