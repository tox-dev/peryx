...
async fn test_concurrent_upstream_attestation_cache_misses_share_one_fetch() {
    ...
    let (request_started, request_received) = tokio::sync::oneshot::channel();
    let request_started = std::sync::Mutex::new(Some(request_started));
    let release_response = std::sync::Arc::new(std::sync::Barrier::new(2));
    let responder_release = release_response.clone();
    Mock::given(method("GET"))
        .and(path(format!("/integrity/{FILENAME}.provenance")))
        .respond_with(move |_: &wiremock::Request| {
            request_started.lock().unwrap().take().unwrap().send(()).unwrap();
    ...
    let release = tokio::task::spawn_blocking(move || {
        request_received.blocking_recv().unwrap();
