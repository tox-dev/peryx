...
fn test_dispatch_changelog_last_serial_queries_only_the_head() {
    reset_source_calls();
    let (last_serial, since_serial) = success_sources();
    assert_eq!(
        (
        ...
        (render_last_serial_response(42), 1)
    );
}
...
fn test_dispatch_changelog_since_serial_passes_the_warehouse_limit() {
    ...
    assert_eq!(
        ...
        (render_changelog_response(&[expected]), 1)
    );
}
...
fn test_dispatch_changelog_request_maps_client_faults(#[case] body: &[u8], #[case] code: i32, #[case] message: &str) {
    ...
    assert_eq!(
        (
            dispatch_changelog_request(body, last_serial, since_serial).unwrap(),
            source_calls(),
        ),
        (render_changelog_fault(code, message), 0)
    );
}
...
fn test_dispatch_changelog_request_maps_invalid_utf8_to_a_parse_fault() {
    ...
    assert_eq!(
        (
            dispatch_changelog_request(&[0xff], last_serial, since_serial).unwrap(),
            source_calls(),
        ),
        (render_changelog_fault(-32700, "parse error; not well formed"), 0)
    );
}
...
thread_local! {
    static SOURCE_CALLS: Cell<usize> = const { Cell::new(0) };
}

fn reset_source_calls() {
    SOURCE_CALLS.set(0);
}

fn source_calls() -> usize {
    SOURCE_CALLS.get()
}

type LastSerialSource = fn() -> Result<u64, ()>;
type SinceSerialSource = fn(i64, usize) -> Result<ChangelogPage, ()>;

fn success_sources() -> (LastSerialSource, SinceSerialSource) {
    (
        || Ok(last_serial_source()),
        |serial, limit| Ok(since_serial_source(serial, limit)),
    )
}

fn last_serial_source() -> u64 {
    SOURCE_CALLS.set(source_calls() + 1);
    42
}

fn since_serial_source(serial: i64, limit: usize) -> ChangelogPage {
    SOURCE_CALLS.set(source_calls() + 1);
    assert_eq!((serial, limit), (42, CHANGELOG_PAGE_SIZE));
    ChangelogPage::new(serial, 43, vec![source_entry()]).unwrap()
}

fn source_entry() -> ChangelogEntry {
    ChangelogEntry {
        project: "demo".to_owned(),
        version: Some("1.0".to_owned()),
        timestamp: 1_725_534_675,
