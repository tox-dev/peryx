...
async fn test_upload_conformance_accepts_wheel_and_sdist_with_metadata() {
    ...
    for (filename, filetype, bytes) in [
        ("peryxpkg-1.0-py3-none-any.whl", "bdist_wheel", wheel.as_slice()),
        ("peryxpkg-1.0.tar.gz", "sdist", sdist.as_slice()),
    ] {
    ...
    let (status, _, body) = get(&harness.state, "/hosted/simple/peryxpkg/", Some("application/json")).await;
    ...
    for (filename, bytes, metadata_prefix) in [
        ...
        ("peryxpkg-1.0.tar.gz", sdist.as_slice(), "Metadata-Version: 2.2\n"),
    ] {
        let missing = format!("{filename} missing");
        let file = files.iter().find(|file| file["filename"] == filename).expect(&missing);
