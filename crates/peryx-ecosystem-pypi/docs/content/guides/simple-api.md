...
description = "Diagnose api-version 1.0, replace gpg-sig checks with served SHA-256 hashes, and handle trailing-slash redirects."
weight = 7
...
### Inspect the advertised version
...
### api-version 1.0 client behavior
...
The Simple API provides integrity hashes. Every file object lists a `hashes` map, and peryx serves a `sha256` for each
...
[PyPI deprecated GPG signatures in 2023](https://blog.pypi.org/posts/2023-05-23-removing-pgp/) and no longer serves
them. A private signing upstream requires direct network access. Requiring its deprecated signature adds an origin
dependency.

## Follow the trailing-slash redirect

A tool that requests `.../simple/{project}` without the trailing slash receives a `301` to the canonical URL from both
pypi.org and peryx. Build the canonical URL to avoid the extra request.
...
### Reject names containing a slash
...
- Serving rationale: [Simple API serving](@/serving.md)
- End-to-end examples: [Simple API behavior](@/tutorials/simple-api-behavior.md) </content>
