...
[upload compatibility](@/uploads.md) for the design and [HTTP endpoints](@/reference/endpoints.md).
...
[Warehouse](https://pypi.org/) (pypi.org) do. For why, see [un-normalized wheels](@/uploads.md#un-normalized-wheels).
...
Publisher identity. See [Simple API serving](@/reference/simple-api.md#provenance-and-attestations) for the served
shape.
...
- The standards these implement: [standards](@/reference/standards.md)
- The full set of upload checks in one place: [publish packages](@/guides/publish.md)
...
- Why peryx accepts these inputs: [what peryx accepts on upload](@/uploads.md) </content>
