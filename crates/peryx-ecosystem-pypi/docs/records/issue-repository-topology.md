...
Make hosted, proxy, and virtual PyPI repositories first-class peryx concepts.

Status: implemented as cached, hosted, and virtual indexes. See [PyPI index roles](@/overview.md#pypi-index-roles). The
text below records the original proposal and its former names.

## Problem

peryx has local, mirror, and overlay indexes in configuration. Those cover the basic mechanics, but they do not give
...
- Add public repository concepts:
  - `hosted`: local packages users upload to peryx
