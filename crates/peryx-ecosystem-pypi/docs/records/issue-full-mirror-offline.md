...
Status: implemented. See [Mirroring](@/reference/mirroring.md) and
[Serve a restricted or air-gapped network](@/guides/air-gapped.md). The text below records the original proposal.

Depends on #28 and #30.

## Problem

peryx is a read-through cache. It fetches project pages and artifacts when pip, uv, or a browser asks for them. That
...
users to choose between mirroring everything and writing scripts around peryx.
...
peryx should support persistent config and CLI overrides.
...
or generated requirements file that already contains that closure. peryx can parse selectors and mirror matching
projects and files.
...
- Add offline serve mode for a repository. In offline mode, peryx must fail closed instead of contacting upstream.
...
- `mirror plan` reports what peryx would fetch without writing new cache entries.
...
- pip and uv can install selected packages from peryx after upstream shutdown.
