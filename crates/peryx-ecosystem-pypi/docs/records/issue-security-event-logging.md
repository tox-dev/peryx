...
Status: implemented through core event contracts and PyPI event fields. See
[Availability logging](@/reference/availability.md#logging). The text below records the original proposal.

## Problem

peryx has operational logs, but it does not define which repository and security actions must be logged or which fields
...
This issue replaces a separate audit-log store for now. peryx should keep using the existing logging pipeline and make
