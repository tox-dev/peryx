...
Status: implemented. See [Policy settings](@/reference/policy.md) and
[Shadowed candidates](@/reference/shadowed-candidates.md). The text below records the original proposal.

Depends on #28.

## Problem

peryx overlays can mix local packages with mirrored upstream packages, but operators cannot state fallback behavior as a
