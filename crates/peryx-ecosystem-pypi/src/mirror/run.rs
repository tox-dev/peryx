...
pub(super) async fn pypi_sync(
    ...
    for project in &selection.projects {
        summary.projects += 1;
        match crate::cache::materialize_detail(state.serving.clone(), target.position, project.clone()).await {
            Ok(Some(_)) => {
...
async fn sync_file(
    ...
    let path = crate::cache::file_path(
        state.serving.clone(),
        digest.clone(),
        target.route.clone(),
        file.filename.clone(),
...
async fn sync_metadata(
    ...
    Ok(SyncOutcome::Downloaded(
        crate::cache::metadata_bytes(&state.serving, &artifact, route, metadata_filename)
            .await?
            .len() as u64,
    ))
}
