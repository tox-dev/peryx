...
pub(super) fn admit(
    ...
    admit_staged(meta, durability, limits, request, now)
        .unwrap_or_else(|err| Admission::Reject(Box::new(staging_failed(&err))))
}
...
fn durability_gate(durability: DurabilityCapabilities) -> Result<(), Admission> {
    durability
        ...
        .map_err(|shortfall| Admission::Reject(Box::new(unsupported_durability(shortfall))))
}
...
fn stage_gate(result: IntentStageResult, filename: &str) -> Result<(), Admission> {
    match result.outcome {
        ...
        IntentStageOutcome::Conflict => Err(Admission::Reject(Box::new(conflicting_content(filename)))),
        IntentStageOutcome::RejectedOverRecordLimit => Err(Admission::Reject(Box::new(shed("record")))),
        IntentStageOutcome::RejectedOverByteLimit => Err(Admission::Reject(Box::new(shed("byte")))),
    }
}
