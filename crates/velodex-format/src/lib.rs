...
//! velodex implements only the Python ecosystem today (the `velodex-ecosystem-pypi` crate); further
//! ecosystems (npm, crates, OCI, …) are sibling crates that add an [`Ecosystem`] variant and an
//! [`EcosystemDriver`] impl without reworking the crates that depend on this one.
